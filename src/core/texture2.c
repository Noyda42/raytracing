...
/*   Updated: 2019/07/01 12:06:44 by temehenn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (fseek(fimg, 10, SEEK_SET) < 0 && !fclose(fimg))
		return (SEEK_IMG_FAIL);
	fread(&head->offset, sizeof(int), 1, fimg);
	if (!is_valid_fread(fimg) && !fclose(fimg))
		return (FREAD_FAIL);
	if (fseek(fimg, 18, SEEK_SET) < 0 && !fclose(fimg))
		return (SEEK_IMG_FAIL);
	fread(&head->w, sizeof(int), 1, fimg);
	if (!is_valid_fread(fimg) && !fclose(fimg))
		return (FREAD_FAIL);
	if (fseek(fimg, 22, SEEK_SET) < 0 && !fclose(fimg))
		return (SEEK_IMG_FAIL);
...
int				load_img(char *image_path, t_text *head)
{
	...
	if (mn[0] != 'B' || mn[1] != 'M')
		return (BAD_IMG_TYPE);
	if ((ret = fill_head(head, fimg)) < 0 && !fclose(fimg))
		return (ret);
