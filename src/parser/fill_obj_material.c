...
static int		fill_obj_material_4(char *line, t_obj *obj)
{
	if (is_token(line, "direction"))
	{
		if (!get_vec(obj->dir, line))
			return (E_ERROR);
	}
	else if (is_token(line, "colord"))
	{
		if (!get_color(obj->colord, line))
			return (E_ERROR);
	}
	return (E_SUCCESS);
}
...
static int		fill_obj_material_3(char *line, t_obj *obj)
{
	...
	{
		if (!fill_obj_material_4(line, obj))
			return (E_ERROR);
	}
	return (E_SUCCESS);
}
